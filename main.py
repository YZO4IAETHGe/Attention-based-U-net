...
img = nib.load("data/MR-dataset/40-T1DUALin-src.nii.gz")
input= img.get_fdata()

img = nib.load("data/MR-dataset/40-T1DUALout-src.nii.gz")
...
output_hat = output_hat.permute(0, 2, 3, 1)
...
axes[0].imshow(input1[0,:, :, input1.shape[3] // 2], cmap="gray")
axes[0].set_title("in")

axes[1].imshow(output_hat[0,:, :, output_hat.shape[3] // 2], cmap="gray")
